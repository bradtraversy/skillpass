...
- [ ] 5. **Submission draft flow** - GitHub OAuth, maintainer profile, GitHub-URL submission (primary) and zip upload (fallback), Neon draft records, R2 snapshot storage.
  ...
  - [ ] 5b. **GitHub-URL submission to draft** - authed `POST /submissions`: validate the URL, pin the commit, snapshot to R2 with a source hash, create the draft record, read endpoints.
  - [ ] 5c. **Zip upload + submit page** - zip fallback through the same snapshot path, plus the `/submit` static shell and React island form wired to the API.
