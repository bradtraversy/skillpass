...
describe('detectionSummary', () => {
	it.each([
		...
		[
			'an inferred manifest',
			{ ...base, manifest: 'inferred' as const },
			'text-pass',
			'SKILL.md found - will list as "demo"',
		],
		[
			'an invalid manifest',
			{ ...base, manifest: 'invalid' as const },
			'text-warn',
			'skill.json is not valid - validation will flag it',
		],
		[
			'a missing SKILL.md',
			{ ...base, manifest: 'missing' as const, skillMd: false, name: null },
			'text-warn',
			'no SKILL.md at the top level - validation will fail; submit the folder that contains it',
		],
